...
import { ShoppingBag, Search, Calendar, DollarSign, User } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {filtered.map(purchase => (
          <div 
            key={purchase.id} 
            className="p-4 bg-white rounded-2xl border border-gray-100 space-y-3"
          >
            <div className="flex justify-between items-start">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <Link to={`/customers/${purchase.customerId}`} className="font-bold text-gray-900 flex items-center hover:text-blue-600 transition-colors">
                    {purchase.customerName}
                  </Link>
                  <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">{format(purchase.date, 'MMM dd, yyyy • hh:mm a')}</p>
                </div>
              </div>
