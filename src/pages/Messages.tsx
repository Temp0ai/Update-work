  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-gray-500">Engage your customers with targeted messages.</p>
      </header>
          ...
          <div className="space-y-2">
            {bdays.map(c => (
              <OutreachCard 
                ...
                message={`Happy Birthday ${c.name}! 🎉 Wishing you a great year ahead.`} 
                type="birthday"
              />
            ))}
          </div>
          ...
          <div className="space-y-2">
            {lapsed.map(c => (
              <OutreachCard 
                ...
                customer={c} 
                purchases={purchases.filter(p => p.customerId === c.id)}
                message={`Hi ${c.name}, we haven't seen you in a while! We've got some new items you might like. Hope to see you soon!`} 
                type="followup"
              />
            ))}
          </div>
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-gray-400 mb-2">Universal template for promotional outreach.</p>
        <div className="bg-white p-4 rounded-2xl border border-gray-100 space-y-4">
           <div className="relative">
             <textarea 
               className="w-full text-sm bg-gray-50 p-3 rounded-xl border border-gray-50 text-gray-600 h-24 focus:ring-2 focus:ring-blue-100 focus:outline-none transition-all"
               defaultValue="Exclusive Offer! 🎁 Enjoy 10% OFF on your next purchase. Use code: CONNECT10. Valid for this week only!"
               id="bulk-template"
             />
           </div>
           <button 
             className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold flex items-center justify-center space-x-2 shadow-lg shadow-blue-100 active:scale-95 transition-all"
             onClick={() => {
            ...
            >
             ...
             <span>Start Bulk Send (4s Delay)</span>
           </button>
        </div>
      </section>
  ...
  useEffect(() => {
    ...
    if (queue.active && !queue.isPaused && queue.currentIndex < queue.customers.length) {
      if (countdown > 0) {
        ...
        // Time to send!
        const link = getWhatsAppLink(currentCustomer.phone, queue.message);
        window.open(link, '_blank');
        
        // Move to next after a bit more delay or immediately? User asked for 4s pause each.
        // We'll set the countdown for the NEXT one.
      ...
      >
        <div className="p-8 space-y-6 text-center">
          ...
          <div className="space-y-1">
            <h3 className="text-xl font-bold">Sending Bulk Messages</h3>
          ...
          {queue.currentIndex < queue.customers.length && (
            <div className="bg-blue-50 py-3 px-4 rounded-2xl flex items-center justify-center space-x-2">
              {countdown > 0 ? (
                <>
                  <Clock size={18} className="text-blue-600 animate-pulse" />
                  <span className="font-bold text-blue-700">Next in {countdown}s...</span>
                </>
              ) : (
                <span className="font-bold text-blue-700">Opening WhatsApp...</span>
              )}
            </div>
          )}
          ...
          <div className="flex flex-col space-y-3 pt-2">
            <button 
              ...
              className={cn(
                ...
                queue.isPaused ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"
              )}
            >
              {queue.isPaused ? 'Resume Sending' : 'Pause Queue'}
            </button>
            <button 
              onClick={onClose}
...
function OutreachCard({ customer, message, type, purchases }: { customer: Customer; message: string; type: 'birthday' | 'followup' | 'offer'; purchases: Purchase[]; key?: string }) {
  ...
  return (
    <div className="bg-white p-4 rounded-2xl border border-gray-100 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={cn(
            ...
            showRefine ? "bg-blue-600 text-white" : "text-blue-600 hover:bg-blue-50"
          )}
      ...
      {showRefine && (
        <div className="p-3 bg-blue-50/50 rounded-xl space-y-3 border border-blue-50">
          <div className="grid grid-cols-2 gap-2">
            <select 
              value={aiOptions.tone}
              onChange={(e) => setAiOptions({...aiOptions, tone: e.target.value as any})}
              className="bg-white border-none rounded-lg text-[10px] py-1.5 px-2 focus:ring-1 focus:ring-blue-100"
            >
          ...
          <input 
            ...
            className="w-full bg-white border-none rounded-lg text-[10px] py-1.5 px-2 focus:ring-1 focus:ring-blue-100"
          />
          <button
            onClick={handleGenerateAI}
            disabled={isGenerating}
            className="w-full py-2 bg-blue-600 text-white text-[10px] font-bold rounded-lg flex items-center justify-center space-x-2"
          >
      ...
      <a
        href={getWhatsAppLink(customer.phone, currentMessage)}
        target="_blank"
        rel="no-referrer"
        onClick={() => setSent(true)}
        className={cn(
          "w-full flex items-center justify-center space-x-2 py-3 rounded-xl text-sm font-bold transition-all",
          sent ? "bg-green-50 text-green-600" : "bg-blue-600 text-white shadow-lg shadow-blue-100 active:scale-95"
        )}
      >
        {sent ? (
          <>
